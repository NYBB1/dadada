	...
	<div>
		<div class="register-container">
			...
			<el-form v-if="pageFlag=='register'" ref="ruleForm" class="rgs-form animate__animated animate__backInDown" :model="ruleForm" :rules="rules">
				<div class="rgs-form2">
					<div class="title">基于智能算法的考试成绩数据分析系统</div>
					<el-form-item class="list-item" v-if="tableName=='xuesheng'">
						<div class="lable" :class="changeRules('studentid')?'required':''">学号：</div>
						<el-input  v-model="ruleForm.studentid"  autocomplete="off" placeholder="学号"  type="text"  />
					</el-form-item>
					<el-form-item class="list-item" v-if="tableName=='xuesheng'">
...
export default {
	...
	mounted(){
		...
		if(this.$route.query.pageFlag=='register'){
			...
			if ('xuesheng' == this.tableName) {
				this.rules.studentid = [{ required: true, message: '请输入学号', trigger: 'blur' }]
			}
			if ('xuesheng' == this.tableName) {
	...
	methods: {
		...
		login() {
			...
			if((!this.ruleForm.studentid) && `xuesheng` == this.tableName){
